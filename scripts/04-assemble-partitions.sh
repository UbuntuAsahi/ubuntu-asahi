...
parted pop-os.img mkpart primary fat32 1MiB 300MiB 2>&1| capture_and_log "create esp"
parted pop-os.img set 1 esp on 2>&1| capture_and_log "make esp bootable"
parted pop-os.img mkpart primary ext4 300MiB 100%

# Get loopback partitions
LODEV="$(sudo losetup --find --show --partscan pop-os.img)"

info "Formatting ESP as FAT32"
sudo mkfs.vfat -F 32 -n EFI "${LODEV}p1" 2>&1| capture_and_log "format esp"

info "Formatting root partition as ext4"
sudo mkfs.ext4 -L "Pop!_OS" "${LODEV}p2" 2>&1| capture_and_log "format rootfs"